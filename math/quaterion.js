...
export class Quaternion {
  ...
  copyFromMatrix4x4(from) {
    let scale = new Vector3().copyFromMatrix4x4Scale(from).inverse();
    ...
    if (trace > 0) {
      ...
      this.set((fromScaled5 - fromScaled7) / S, (fromScaled6 - fromScaled2) / S, (fromScaled1 - fromScaled3) / S, 0.25 * S);
    } else if (fromScaled0 > fromScaled4 && fromScaled0 > fromScaled8) {
      S = Math.sqrt(1.0 + fromScaled0 - fromScaled4 - fromScaled8) * 2;
      this.set(0.25 * S, (fromScaled1 + fromScaled3) / S, (fromScaled6 + fromScaled2) / S, (fromScaled5 - fromScaled7) / S);
    } else if (fromScaled4 > fromScaled8) {
      S = Math.sqrt(1.0 + fromScaled4 - fromScaled0 - fromScaled8) * 2;
      this.set((fromScaled1 + fromScaled3) / S, 0.25 * S, (fromScaled5 + fromScaled7) / S, (fromScaled6 - fromScaled2) / S);
    } else {
      S = Math.sqrt(1.0 + fromScaled8 - fromScaled0 - fromScaled4) * 2;
      this.set((fromScaled6 + fromScaled2) / S, (fromScaled5 + fromScaled7) / S, 0.25 * S, (fromScaled1 - fromScaled3) / S);
    }

    return this;
  }

  setAxisAngle(axis, angleRadians) {
    angleRadians /= 2;
    let s = Math.sin(angleRadians);
    this.set(s * axis.x, s * axis.y, s * axis.z, Math.cos(angleRadians));
    return this;
  }
  ...
  angle(other) {
    return Math.acos(2 * Math.pow(this.dot(other), 2) - 1);
  }
  ...
  calculateW() {
    this.w = Math.sqrt(Math.abs(1.0 - Math.pow(this.x, 2) - Math.pow(this.y, 2) - Math.pow(this.z, 2)));
    return this;
  }
  ...
  naturalLog() {
    ...
    this.y *= t;
    this.z *= t;
    this.w = 0.5 * Math.log(Math.pow(this.x, 2) + Math.pow(this.y, 2) + Math.pow(this.z, 2) + Math.pow(this.w, 2));
    return this;
  }
  ...
  copy(from) {
    this.set(from.x, from.y, from.z, from.w);
    return this;
  }
  ...
  slerp(other, amount) {
    ...
    return this.set(scale0 * this.x + scale1 * bx, scale0 * this.y + scale1 * by, scale0 * this.z + scale1 * bz, scale0 * this.w + scale1 * bw);
  }
  ...
  invert() {
    ...
    dot = 1 / dot;
    return this.set(-this.x * dot, -this.y * dot, -this.z * dot, this.w * dot);
  }

  conjugate() {
    this.set(-this.x, -this.y, -this.z, this.z);
    return this;
  }
  ...
  fromEuler(x, y, z) {
    ...
    let sz = Math.sin(z);
    let cz = Math.cos(z);
    return this.set(sx * cy * cz - cx * sy * sz, cx * sy * cz + sx * cy * sz, cx * cy * sz - sx * sy * cz, cx * cy * cz + sx * sy * sz);
  }

  fromEulerVector(from) {
